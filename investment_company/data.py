...
def compute_indicators(history: pd.DataFrame, windows: Optional[List[int]] = None) -> pd.DataFrame:
    ...
    min_required = max(min(windows) if windows else 1, 14, 20, 26)
    df.attrs["insufficient_history"] = len(history) < min_required or df.empty
    df.attrs["observations"] = len(history)
    df.attrs["min_required"] = min_required
    return df
