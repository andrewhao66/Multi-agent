...
def backtest_portfolio(decision: Dict, data: PriceData) -> BacktestReport:
    ...
    avg_return = (valid_returns.mean() * periods_per_year * weight) if not valid_returns.empty else 0.0
    if valid_returns.empty:
        volatility = 0.0
    else:
        volatility = float(valid_returns.std(ddof=0) * sqrt(periods_per_year) * max(weight, 1e-9))
